 ...
 * ⚠⚠  未完成的草稿 —— 不要照它的输出下结论  ⚠⚠
 *
 * 这份遍历器是自动生成的初稿，**尚未经过审阅**，而且已知有两条断言是坏的：
 *
 *   A1「效果声明 vs 真能落地」：独立证伪的结论是**这条规则作为检测器是坏的** ——
 *      四个检查里两个永远不可能触发，其中一个真触发了还是误报；
 *      而且主检查在最关键的那 19 个 units:"turns" 动作上会 **100% 漏报**。
 *   A2「区域几何退化」：报出的数字与实测口径全部对不上（同一类问题此前已让本项目
 *      两次把错误统计写进发行版，见 CHANGELOG 0.2.2）。
 *
 * 其余六条断言未被独立核实。**修好 A1/A2 并逐条核实之前，这个文件只作参考。**
 * 它不进发布件（release 的 zip 只打包 module.json / README / CHANGELOG / LICENSE / scripts / docs）。
 * ============================================================================ */
