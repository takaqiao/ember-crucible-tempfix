...
**174 条断言**，含大量反向断言（上游修好了就别动、只按 id 命中、ember 的钩子不能被顶掉、
关掉开关后行为回到上游原样）。当前：**174 passed / 0 failed**。

桩件本身也验过 —— 变异测试把 43 处补丁逐个改回坏写法，**43/43 全部被抓住**。
...
1. **全部 26 条都只有静态取证 + 桩件测试，一次真实 Foundry 验证都没做过。**
...
1. **开世界跑第 3 节那张表。** 26 条补丁一条真实验证都没有，这是唯一该先做的事。
2. 按实测结果订正本文件第 4 节。
