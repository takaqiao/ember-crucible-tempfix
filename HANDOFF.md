...
```powershell
node "C:\Users\Taka\Desktop\fvtt\ember-crucible-tempfix\tests\upstream_check.mjs"
```

对着**真正安装的** crucible / ember，把每条补丁的上游判据挨个试一遍，回答一个问题：
**哪些补丁因为上游改了实现而停止工作了，其中哪些是意料之外的。**
harness 是对着桩件验补丁逻辑，它不知道上游长什么样；这个脚本补的正是那一半。

失配分两类，混着看会出人命：

- **失配 + 已登记 `VERSION_CEILINGS`** → 预期内的退休，好事
- **失配 + 没登记上限** → 补丁已**静默停止工作**，而缺陷可能还在（脚本以退出码 1 报警）

判定强度**不对称**，别越级解读：原型补丁的判据运行时读的是 `String(proto[guard.method])`，
所以脚本也把搜索收进那个方法体，两个方向的结论都成立；
动作补丁的 `__guard` 读的是 compendium 里的钩子源码，静态取不到，只能全库搜 ——
全库**搜不到**能推出方法里也没有，**搜得到则什么都推不出**。所以那类只报失配，不报「仍命中」。

脚本末尾自报覆盖率（哪些补丁它没查、各自靠哪条轴兜底），别把「全绿」当成「全查过」。

**选特征串的规矩**（0.10.2 这轮体检得出的）：串必须选**上游修好后会消失**的东西。
反例两条，都是被这个脚本抓出来的：

- **I5** 用 `game.user.isGM` —— 全库 138 处，修不修都在，guard 轴等于没有；
- **I4** 用注释 `do not accumulate costs` —— 上游 0.10.2 确实修了，
  却**把旧注释留着、另外加了一句新的**，串原样还在。「注释多半会跟着改」这个赌注输了。

这两条现在只剩版本上限一根独苗兜底。它们已被上限永久退休，不必补救；
但**新加补丁别再这么选串** —— 挑一句只存在于坏实现里的代码。

### 数据轴体检（跟版体检的另一半）

```powershell
node "C:\Users\Taka\Desktop\fvtt\ember-crucible-tempfix\tests\data_check.mjs"
```

⚠ 需要 **Foundry 没在跑**（LevelDB 单写锁）。打不开的 pack 会被列出来，那时的数是不完整的。

三条轴，三个体检，别混：

| 脚本 | 管什么 | 上游一变会怎样 |
|---|---|---|
| `tempfix_harness.mjs` | 补丁逻辑本身（对着桩件） | 不知道上游长什么样，不会变红 |
| `upstream_check.mjs` | **代码轴** —— `__guard` 特征串还命不命中 | 失配即报警 |
| **`data_check.mjs`** | **数据轴** —— pack 里的标签 / change key / 时长单位 | 事实翻转即报警 |

数据轴的补丁（D-1/2/3、E1）靠**形状匹配**自动空转，既没有 guard 也没有版本上限 ——
上游把数据修好了**不会有任何信号**，补丁就一直空转下去，没人知道它该退休了。
这个脚本就是那个缺失的信号。

它同时把几个**历史上算错过的数**变成可复算的：

- **N10 影响面**：报「不同动作 id 数 19」与「副本总数 22」**两个数**，
  外加 `crucible 侧 7 + ember 侧 13 − 两边都有的 1（steamVent）= 19`。
  当年 19 → 38 → 19 的来回，根子就是把这三种口径混着说。**引用时写清楚是哪一个。**
- **副本数 vs 提及数**：冒险包正文的 `@UUID`、敌人的物品授予表都会提到同一个 id，
  子串搜索因此会多数出 pack 来（`noxiousSpray` 子串命中 5 个 pack，真副本只有 3 份）。
  脚本把差额单列，并声明：**差额若不能全部解释成引用，就是结构化判据漏掉了某种形状的
  副本，属于真警报。** D-1「三份副本都已修好」这个结论正是靠这一栏才敢下 ——
  它曾两次差点被误判成「`patchDamageTypes` 可以整条退休」，
  而真相是三个格子里只有它一个修好了，`selfDestruct` 与 `devourThoughts` 还坏着。
...
0. **先把两个离线体检跑一遍**（`upstream_check.mjs` / `data_check.mjs`，见第 3 节）。
   不需要开世界，几秒钟就能知道“上游动过什么、哪条补丁静默失效了”。
1. **开世界跑第 3 节那张表。** 除 N10 的缺陷侧外一条真实验证都没有，这是唯一该先做的事。
   先跑扫描器那一步（自动消掉 11 个开关的机制半边），带 ⏸ 的 11 行不必验，
   剩下 **11 个开关**才需要真上牌桌。
   建议从 `patchTurnsDuration` 起手：它的缺陷侧已经确认，是唯一一个能拿现实做对照的。
   ⭐ 其次优先 `patchThrowableOnly` 的 **I7′** 行 —— 那一格的行为 0.8.2 才真正存在，
   以前的绿是桩件造的假象。
2. 按实测结果订正本文件第 4 节。
