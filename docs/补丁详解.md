...
> 这份文档回答「**为什么**这么修」。想知道「修了什么、现在什么状态」看 [README](../README.md)；
> 想看行号级的完整取证过程看 [`上游缺陷诊断.md`](上游缺陷诊断.md)。
>
> 每条结论都带 `crucible-compiled.mjs` / `ember.mjs` 的行号。行号对应
> **Crucible 0.10.1**（多数已在 0.10.2 上复核）与 **Ember 0.6.0 / 0.6.1**，上游改版后可能漂移。

---

## P1 · 副手攻击的前置判据（含徒手）

**症状**：打完主手，点副手攻击，提示「必须紧跟一次主手打击」。**赤手空拳时必然发生。**
...
**症状**：血统给了符文（比如泽夫给风暴符文）却没给对应动作；召唤出来的精怪不但没有戏法，
施放本命符文的法术还按「未受训 −4」算。

**根因**：`system.rune` 只负责把符文塞进 `grimoire.runeIds`（`:41287`），
而每个符文的**招牌戏法**挂在 crucible 那条 `Rune: X` 天赋的 `system.actions` 上。

- **ember 四个血统**（泽夫 Zeph / 龙裔 Drakon / 基斯卡 Kiska / 尼尔艾 Nir'ae）一个都没带，其中三个还漏了 `training` 阶位。
...
**本模块的做法**：按 **rune** 查表（不是按天赋 item id）；
戏法的数据从 `crucible.talent` 合集**读**出来，所以名字与描述会带上 babele 的中文。
玩家自己已经学了 `Rune: X`、或条目本身就带该动作时不会重复添加。

> `life`(Healer) / `soul`(bard) 故意不在表里 —— 它们的**正版条目**训练阶位本来就是空的，属于设计。

### ⚠ 上面这两批性质不同，0.7.4 起是两个开关

一次对抗式审计（重推全部补丁前提）指出：上面第一条和第二条**依据的强度完全不一样**。

| | 依据 | 性质 | 开关 |
|---|---|---|---|
| `crucible.summons` 的 9 条旧快照 | `actions` 是空数组，而 `crucible.talent` 里**同名正版**有 —— 同一条目两份数据自相矛盾 | **可证的缺陷** | `patchRuneCantrips` |
| ember 四个血统 | **无** | **内容判断** | `patchLineageCantrips` |

第二条站不住的原因很硬：**crucible 全文 `cantrip` 零命中**，
`RUNES` 配置（`:5788` 起，逐条只有 id/name/img/resource/damageType/opposed/defense/scaling/nameFormat）
**没有戏法字段** —— 系统从未建立「符文 ⇒ 戏法」这个关联。
而那四个血统各自带着自己的招牌动作（`emberZephLineage` 有莫测/慑人/蛊惑三个面容，
`emberDrakonLinea` 有 `drakonfire`……），它们不是「坏掉的副本」，只是没包含那个戏法。

给它们补戏法是**内容取舍**，不是修 bug —— 这违反了本模块自己定的排除规则
（见 `上游缺陷诊断.md` §7「缺的都是设计数据，补丁猜不出来」）。

代码里的判据就是这个区别：**这个天赋自己有没有动作**。没有 = 坏掉的快照，有 = 另一种东西。
两条都默认开启（行为与 0.7.3 一致），但现在可以分开关掉。
...
**症状**：暴击后用「湮解印记」，走完对话框**什么都不发生** —— 不扣资源、不生成聊天卡，
...
本机 crucible 0.10.2 + ember 0.6.1 全部 packs 深扫，按不同动作 id 去重共 **19 个**：

| 来源 | 个数 | 例 |
|---|---|---|
| **crucible 自身** | **7** | `devourThoughts`、`mindFlay`、`eldritchEmanation`、`ferociousHowl`、`pestilentLash`、`shieldBash`、`steamVent` |
| ember 敌手 / 消耗品 | 13 | `alchemicalGrenade`、`frostFlask`、`electroAmpoule`、`cosmicGem`×3、`lightningBurst`、`flamingSpray`、`restrainingBolt`、`deathThroes`、`drakonbanePoisonIngest`、`fingerWakingNightmares`、`steamVent` |

（`steamVent` 两侧都有，所以分项和 20 > 19。）**crucible 自己那 7 个现在是主体** ——
这不只是「ember 数据配 crucible 校验」的接缝问题，crucible 单机也踩。

### ⚠ 这个数字被改过两次，方向相反

| 何时 | 数字 | 怎么回事 |
|---|---|---|
| 最早 | 19 | **漏数**：只统计未迁移的 `{turns:N}` 旧形，漏了已迁成 `units:"turns"` 的新形，也漏了冒险包里 actor 内嵌物品上的独立实例 |
| 0.6.0 时 | **38** | 补全后的真数（ember 32 + crucible 7，`steamVent` 两侧共有） |
| **Ember 0.6.1 起** | **19** | 上游自己迁走了一大半（changelog：「Fixed an issue with "turns"-unit durations, leading to them never applying」） |

**「九个血统的招牌变身全部中招」这句话已经作废** —— Ember 0.6.1 把
泰拉菲克变形 / 结晶化创伤 / 极限代谢 / 活石 / 规整节律 / 刺棘树皮 / 强健体力 /
无情猎手 / 泽夫三面容全部迁成了 `{units:"rounds", expiry:"turnEnd"}`，数值保留。
`crucible-character` 包里现在 26 条效果时长、**0 条是 turns**。

> **顺带一件值得记的事**：上游迁移用的正是 `rounds` + `expiry:"turnEnd"` + **数值不变**
> （`implacableHunter` 的 `value: 360` 原样保留），与本模块的映射**逐字一致**。
> 0.7.4 时一次审计指出「`expiry` 取 `turnEnd` 本机无法复核」——
> 现在上游用自己的迁移结果追认了它。

另有 **25 个**同样坏掉的效果直接写在**物品文档**自己的 `effects[]` 上（例如神话尖塔守护者
Mythspire Guardian 的天赋「濒临死亡 Nearing Death」上的效果「消损之毒 Wasting Poison」，
`transfer:true`，持有即应生效却永远建不出来）。它们的创建不经过动作，`preActivate` 够不着 ——
由 **N12** 在效果创建那一层补，与 N10 共用 `patchTurnsDuration` 开关。

**这一半 0.6.1 反而从 22 涨到 25** —— 新加的怪物仍在用 turns 写。
也就是说 N10/N12 短期内退不了休。
...
> （**N3 斥退踢击仍用 `turnStart`**：它的数据不是 turns 型，最近的权威是
...
> **已知未修**：「泰拉菲克变形」的第三条加成（威吓 +2 恩惠骰）没有补 ——
> `rollBonuses` 每轮被重置成恰好 `{damage:{}, boons:{}, banes:{}}`（`:41179`），
> 恩惠骰的读取点（`:36852`）是全局的、**无法按技能限定**。

---

## N3 · 「斥退踢击」的踉跄变永久

**症状**：被哨卫「斥退踢击」命中的角色，`staggered` **永不消失**，
而 `:41808` 使该角色**每回合永久少 2 点动作点**。效果卡渲染成 ∞ 归入 persistent 段，极易被当成设定。
...
代价还含 2 点英雄气概。对照 crucible.spell 的 `Revive` 完全同构但 `scope: 4`，靠 `isDead` 闸门做门禁。
...
**症状**：装备并投注后，控制台每次数据准备刷一条错误；点「暗焰光束」→
**什么都不发生**，资源不扣、聊天卡不生成。
...
面板上的「运行自检」跑的就是它（命令表见 [README](../README.md#控制面板)）。
选中你的 token 之后：
...
**补丁侧** —— `_tests` / `callActorHooks` 两个包装装没装上、当前启用哪几个 action id、
`HOOK_OVERRIDES` 的覆盖状态、10 条原型补丁各自「已包装 / 未包装」× 「开 / 关」两条轴、
以及 swallow 的替换 id、N11 修好了几个词缀、夹击开关包没包。

> 「已包装」和「开 / 关」是**两条独立的轴**：包装体永不卸载，
> 所以关掉开关后仍显示「已包装」—— 这是正常的，不是没关掉。

**角色侧**（选中了 token 才有）—— `suddenBite` 范围、疗愈导流的 usage、
惑乱凝视的防御类型、余烬之火花的 scope、反重力石的 target 类型、已注入的戏法、
训练阶位、主副手武器的**存盘 slot vs 派生 slot**（P1 的判据就在这两者的差异上）。

改完设置后模块会自动重跑一遍角色数据准备；也可以手动
`emberCrucibleTempFix.reprepareActors()`，或点面板上的「重新准备角色数据」。
